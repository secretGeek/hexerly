...
(function () {
    ...
    this.drawHex = function (c, xOffset, yOffset, fillStyle, borderColor) {
        ...
        var i, gradient;
        c.beginPath();
        c.moveTo((hexPoints[0][0] + xOffset) * SCALE, (hexPoints[0][1] + yOffset) * SCALE);
        for (i = 0; i < 6; i = i + 1) {
		    c.lineWidth = 1.0;
            c.lineTo((hexPoints[i + 1][0] + xOffset) * SCALE, (hexPoints[i + 1][1] + yOffset) * SCALE);
        }
    ...
    this.drawHexBorder = function (c, xOffset, yOffset, borderColors) {
        ...
        for (i = 0; i < 6; i = i + 1) {
			...
			c.lineJoin= "round";
            c.moveTo((hexInnerPoints[i][0] + xOffset) * SCALE, (hexInnerPoints[i][1] + yOffset) * SCALE);
            c.lineTo((hexInnerPoints[i + 1][0] + xOffset) * SCALE, (hexInnerPoints[i + 1][1] + yOffset) * SCALE);
    ...
    this.columnRowToXY = function (column, row) {
        return [(column - 1) * TILE_WIDTH, (row - 1) * TILE_HEIGHT + ((column) % 2 * (TILE_HEIGHT / 2))];
    };
